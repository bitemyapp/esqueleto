...
--
-- /Since: 3.1.0/
(->.) :: SqlExpr (Value (Maybe Aeson.Value)) -> Either Int Text -> SqlExpr (Value (Maybe Aeson.Value))
...
--
-- /Since: 3.1.0/
(->>.) :: SqlExpr (Value (Maybe Aeson.Value)) -> Either Int Text -> SqlExpr (Value (Maybe Text))
(->>.) value (Right txt) = unsafeSqlBinOp " ->> " value $ val txt
(->>.) value (Left i)    = unsafeSqlBinOp " ->> " value $ val i
...
--
-- /Since: 3.1.0/
(#>.) :: SqlExpr (Value (Maybe Aeson.Value)) -> [Text] -> SqlExpr (Value (Maybe Aeson.Value))
...
--
-- /Since: 3.1.0/
(#>>.) :: SqlExpr (Value (Maybe Aeson.Value)) -> [Text] -> SqlExpr (Value (Maybe Text))
(#>>.)  value = unsafeSqlBinOp " #>> " value . mkTextArray
...
--
-- /Since: 3.1.0/
(@>.) :: SqlExpr (Value (Maybe Aeson.Value)) -> SqlExpr (Value (Maybe Aeson.Value)) -> SqlExpr (Value Bool)
(@>.) = unsafeSqlBinOp " @> "
...
--
-- /Since: 3.1.0/
(?.) :: SqlExpr (Value (Maybe Aeson.Value)) -> Text -> SqlExpr (Value Bool)
...
--
-- /Since: 3.1.0/
(-.) :: SqlExpr (Value (Maybe Aeson.Value)) -> Either Int [Text] -> SqlExpr (Value Bool)
...
fromPersistValueParseError received err = T.concat
    [ "Failed to parse Haskell type `Aeson.Value`, "
    , "but received ", T.pack (show received)
    , " | with error: ", err
    ]
